...
function Roach() {
    ...
    this._jobsDoor.on('open', function(){
        ...
        if (self._transportsDoneDoor.isOpen) {
            self.close();
        }
        else {
...
Roach.prototype.run = function() {
    ...
    if ( !this._transportsReadyDoor.isOpen ){
        ...
        this._transportsReadyDoor.once('open', function(){
    ...
    //     console.log('All trasnports ready', self._jobs);
...
Roach.prototype.addJob = function( url, name, options ) {
    ...
    job.on('job:parsed', function(type, data){

        this.save(data);
    }, this);

    job.on('exit', function(name){
