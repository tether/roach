...
function active(client, id) {
  ...
  client.lindex('roach:jobs:active', id, function(err, res) {
    if(err) return promise.reject(str + 'already active');
    client.lrem('roach:jobs:pending', 0, id, function(err) {
      if(err) return promise.reject( str + 'can not be removed from pending queue');
      client.lpush('roach:jobs:active', id, function(err) {
        if(err) promise.reject(str + 'can not be pushed in active queue');
        promise.resolve();
      });
    });
  });
